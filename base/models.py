from togohydromet.regex import regex_tel, regex_noid
...
class Limite(models.Model):
    ...
    def section_communale(self):
        limite_lst = Limite.objects.filter(typelimite__niveau=4, code=self.code[:7]).first()
        if limite_lst :
...
class Zone(models.Model):
    ...
    nom = models.CharField(max_length=40)
    description = models.TextField(max_length=100, blank=True)
    typezone = models.ForeignKey(TypeZone, blank=True, null=True)
    shape = models.PolygonField(null=True, blank=True)
    timestamp_add = models.DateTimeField(auto_now_add=True)
    timestamp_update = models.DateTimeField(auto_now=True)
    objects = models.GeoManager()
